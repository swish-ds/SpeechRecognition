...
def array_to_img(x, data_format=None, scale=True):
    ...
    x = np.asarray(x, dtype=tf.keras.backend.floatx())
    if x.ndim != 3:
...
def img_to_array(img, data_format=None):
    ...
    x = np.asarray(img, dtype=tf.keras.backend.floatx())
    if len(x.shape) == 3:
        if data_format == 'channels_first':
            x = x.transpose(2, 0, 1)
    elif len(x.shape) == 2:
    ...
    def fit(self, x,
        ...
        if augment:
            ax = np.zeros(tuple([rounds * x.shape[0]] +
                                list(x.shape)[1:]), dtype=tf.keras.backend.floatx())
        ...
        if self.featurewise_std_normalization:
            ...
            x /= (self.std + tf.keras.backend.epsilon())
...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        if data_format is None:
            data_format = tf.keras.backend.image_data_format()
        self.x = np.asarray(x, dtype=tf.keras.backend.floatx())
    ...
    def next(self):
        ...
        batch_x = np.zeros(
            tuple([current_batch_size]+(1,) + list(self.x.shape)[1:]), dtype=tf.keras.backend.floatx()) #Added +(1,) +
        for i, j in enumerate(index_array):
            x = self.x[j]
            x = self.image_data_generator.random_transform(
                x.astype(tf.keras.backend.floatx()))
...
class DirectoryIterator(Iterator):
    ...
    def next(self):
        ...
        batch_x = np.zeros((current_batch_size,)  +(self.frames_per_step,)+       
                           self.image_shape, dtype=tf.keras.backend.floatx())   # # my addition of +(1,)
        ...
        if self.class_mode == 'input':
            ...
            batch_y = self.classes[index_array].astype(tf.keras.backend.floatx())
        elif self.class_mode == 'categorical':
            batch_y = np.zeros(
                (len(batch_x), self.num_class), dtype=tf.keras.backend.floatx())
